...
resultMsg = ""
resultPath = "0601_1/Student_1_{std}/output/json/swing{swing}/output.txt".format(std=std, swing=swing)
...
keyPointVector = []
keyPointVector.append(generateVector('../OpenPose/Golf_Run/realTimeJudge/keyAction_Key/swing1_keypoints.json'))
keyPointVector.append(generateVector('../OpenPose/Golf_Run/realTimeJudge/keyAction_Key/swing2_keypoints.json'))
keyPointVector.append(generateVector('../OpenPose/Golf_Run/realTimeJudge/keyAction_Key/swing3_keypoints.json'))
keyPointVector.append(generateVector('../OpenPose/Golf_Run/realTimeJudge/keyAction_Key/swing4_keypoints.json'))
keyPointVector.append(generateVector('../OpenPose/Golf_Run/realTimeJudge/keyAction_Key/swing5_keypoints.json'))
...
ready_begin_compareVector = generateVector('../OpenPose/Golf_Run/realTimeJudge/SampleReadyKey/sample_keypoints.json') # 取得要比對的向量資訊
#ready_begin_compareVector = generateVector('SampleReadyKey/sample_keypoints.json') # 取得要比對的向量資訊

# lastFiles = glob.glob('Mykey/*')
# for f in lastFiles:
    # os.remove(f)
subprocess.Popen(["../OpenPose/Golf_Run/run/OpenPoseDemo.exe","--model_folder","../OpenPose/Golf_Run/models","--camera","0","--num_gpu_start","0","--process_real_time","-number_people_max","1","--net_resolution","480x368","--write_json","0601_1/Student_1_{std}/output/json/swing{swing}/".format(std=std, swing=swing)])
time.sleep(1)
...
while True:
    ...
    fileName = "0601_1/Student_1_{std}/output/json/swing{swing}/Student_1_{std}_{swing}_".format(std=std, swing=swing)
    nextFileName = "0601_1/Student_1_{std}/output/json/swing{swing}/Student_1_{std}_{swing}_".format(std=std, swing=swing)
