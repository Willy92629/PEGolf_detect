  ...
  function switchSwingDetail(stdIdx, swingIdx) {
    ...
    fetch('./stdscore.json')
      ...
      .then(data => { 
        ...
        document.getElementById("totalScore").innerHTML = stdData.swingScore[swingIdx-1]
        for (var i = 0; i < stdSwingStep.length; i++) {
            var stepStr = `step${i+1}`;
            if(stdSwingStep[i] == 1) {
                document.getElementById(stepStr).innerHTML = "V";
            } else {
                document.getElementById(stepStr).innerHTML = "";
            }
        }
        document.getElementById("swingScore").innerHTML = `${swingScore}`;
        ...
        var carouselInner = document.getElementById("carouselInner");
        var ImgHTML = `<div class="carousel-item active"><<div id="videoContainer" style="display: block;">
        <video controls width="400" height="700">
            <source src="${stdData.swingPics[swingIdx-1]}" type="video/mp4">
        ...
        for(var i = 1; i < stdData.swingCount; i++) {
            ...
            var picPath = stdData.swingPics[i];
            var altStr = `swing ${i+1}`;
            ImgHTML += `<div id="videoContainer" style="display: block;">
                    <video controls width="400" height="700">
